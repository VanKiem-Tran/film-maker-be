...
import type { Creator, MetadataArgs } from '@metaplex-foundation/mpl-bubblegum';
import {
  computeCreatorHash,
  computeDataHash,
...
(async () => {
  ...
  const collectionMasterEdition: PublicKey =
  ...
  const slicedProof: AccountMeta[] = assetProof.proof
    ...
    .slice(0, assetProof.proof.length - (canopyDepth || 0));
