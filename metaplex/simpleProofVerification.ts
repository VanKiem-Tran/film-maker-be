...
import type { GetAssetProofRpcResponse } from '@metaplex-foundation/js';
import { Metaplex, ReadApiConnection } from '@metaplex-foundation/js';
import type { MerkleTreeProof } from '@solana/spl-account-compression';
import {
...
(async () => {
  ...
  const { assetIdTestAddress } = keys;
  const { assetIdUserAddress } = keys;
  ...
  const assetProof = (await metaplex
    .rpc()
    .getAssetProof(assetId)) as GetAssetProofRpcResponse;
  ...
  console.log(
    ...
    MerkleTree.verify(rootFromRpc, merkleTreeProof),
  );
  ...
  console.log(
    ...
    new PublicKey(currentRoot).toBase58() ===
      new PublicKey(rootFromRpc).toBase58(),
  );
})();
